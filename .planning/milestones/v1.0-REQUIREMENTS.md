...
- [x] **AUTH-02**: User can log in with their email and password to start a session.
- [x] **AUTH-03**: User session is maintained securely via JWT.
- [x] **AUTH-04**: User can log out to terminate the session.

### Audit Logging & History (AUDT)

- [x] **AUDT-01**: System records metadata for each masking job (user ID, original file name, file size, row count, and execution status).
- [x] **AUDT-02**: System records which columns were masked and with what rules in each job.
- [x] **AUDT-03**: User can view a dashboard displaying their personal masking job history.
- [x] **AUDT-04**: Job history records contain only metadata and never store the actual contents of the files.
